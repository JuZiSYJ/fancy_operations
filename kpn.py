...
class KPN(nn.Module):
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        # print('core.shape: {} data.shape:{} \n'.format(core.shape, data.shape) )
...
class KernelConv(nn.Module):
    ...
    def _convert_dict(self, core, batch_size, N, color, height, width):
        ...
        # print(core_out[5].shape) Size([4, 3, 25, 1, 224, 224])
    ...
    def forward(self, frames, core, white_level=1.0, rate=1):
        ...
        for index, K in enumerate(kernel):
            if not img_stack:
                ...
                # print('img_stack[0].shape:{}'.format(img_stack[0].shape)) # 25,4*3*1*224*224
                img_stack = torch.stack(img_stack, dim=2) # Size([4, 3, 25, 1, 224, 224])

            else:
                k_diff = (kernel[index - 1] - kernel[index]) // 2
                img_stack = img_stack[:, :, k_diff:-k_diff, ...]
            # print('corr..shape:{}  img_stack.shape{}:'.format(core[K].shape, img_stack.shape))
...
if __name__ == '__main__':
    ...
    # flops, params = get_model_complexity_info(kpn, (3, 224, 224),, as_strings=True, print_per_layer_stat=True)
    # print('{:<30}  {:<8}'.format('Computational complexity: ', flops))
    # print('{:<30}  {:<8}'.format('Number of parameters: ', params))
    #
    #
    # sq = models.squeezenet1_1(pretrained=False)
    # feature  = nn.Sequential(*list(sq.children())[0][:5])
    # x = torch.rand((5,3,224,224))
    # print(feature(x).shape, feature)
